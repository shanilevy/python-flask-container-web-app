pipeline {
  ...
  environment{
      ...
      LOCATION = 'us-west1'
      CREDENTIALS_ID = 'gke'
      TAG = "us-west1-docker.pkg.dev/shanilevy-615-2023063002023400/flask-app/flask-app:1.${env.BUILD_ID}"
  }
  stages {
    stage('Checkout') {
        steps {
    ...
    stage('Deploy to GKE') {
            steps {
                //sh "sed -i 's/TEST_IMAGE_NAME/${TAG}/g' kubernetes_private.yaml"
                ...
                withCredentials([file(credentialsId: 'mongosecret', variable: 'MONGO_S')]) {
                    sh '''
                            sed -i "s/TEST_IMAGE_NAME/${TAG}/g" kubernetes_private.yaml
                    '''
